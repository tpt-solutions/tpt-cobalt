...
fn submodel_anatomy(grid: UniformGrid1D) {
    ...
    let mass0: f64 = vec![0.0_f64; N].iter().sum();
    let mass1: f64 = heat.state().iter().sum();
    println!(
        "    20 x dt=0.01 with a center source: peak = {peak:.4}, \
         mass {mass0:.1} -> {mass1:.3} (Dirichlet leaks mass through the ends)"
    );
    assert!(peak < 1.0, "heated peak must stay bounded");
    assert!(mass1 < mass0 + 1e-6, "Dirichlet must not inject net mass");
}
...
fn assemble_sir() -> (ReactionSystem, SirIdx) {
    let sir = ReactionNetwork::from_dsl(
        "beta, S + I --> 2 I
         gamma, I --> R",
    )
...
fn reproducibility(sim: &mut Simulation, idx: &SirIdx, hist: &History) {
    ...
    // Restore the peak snapshot into a *fresh* run and drive it forward again.
    let before = sim.model("sir").expect("sir").state().to_vec();
    sim.restore(&hist.peak_checkpoint)
        .expect("restore succeeds");
    let after = sim.model("sir").expect("sir").state().to_vec();
    let sir_dev = before
        .iter()
        .zip(&after)
        .map(|(a, b)| (a - b).abs())
    ...
    println!(
        "    restore(peak): SIR state before vs after = {:.2e} (bit-for-bit for the ODE)",
        sir_dev
    );
